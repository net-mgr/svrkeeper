...
def main():
    ...
    ### External Test
    # Internal exsist account => External exsist account
    sender_conf_dct = set_smtpconf()
    receiver_conf_dct = set_gmailconf()

    mail_sender = MailSender(0, sender_conf_dct)
    mail_receiver = MailReceiver(1, receiver_conf_dct)

    ts = datetime.now()
    s_ts = ts.strftime('%Y/%m/%d %H:%M:%S')
    hash = hashlib.sha256(s_ts.encode('utf-8')).hexdigest()

    mail_sender.send_mail(sender_conf_dct['account'], receiver_conf_dct['addr'], 'Test mail', hash)
    msg_contents_dct = mail_receiver.recv_mail(0)

    if msg_contents_dct['body'] == hash:
        #print("OK")
    else:
        print("External Mail Test(Internal => External) Failed:")
        ret = 1

    # Internal exsist account => External non-exsist account
    sender_conf_dct = set_smtpconf()
    ...
    non_exist_addr = os.environ.get('MAIL_EXTERNAL_RECEIVER_NONEXIST_ACCOUNT')
    try:
        ...
        if e.smtp_code == 554:
            ...
            print("NG")
            print("External Mail Test(Internal => External<non-exist>) Failed:")
            ret = 1

    # External exsist account => Internal exsist account
    sender_conf_dct = set_gmailconf()
    receiver_conf_dct = set_receiver_conf()

    mail_sender = MailSender(1, sender_conf_dct)
    ...
    i = 0
    while (i < num_RecvAccounts):
        mail_sender.send_mail(sender_conf_dct['addr'], receiver_conf_dct['addr'][i], 'Test mail', hash)
        msg_contents_dct = mail_receiver.recv_mail(i)

        if msg_contents_dct['body'] == hash:
            #print("OK")
        else:
            print("External Mail Test(External => Internal) Failed:")
            ret = 1

        i += 1

    # External exsist account => Internal non-exsist account
    sender_conf_dct = set_gmailconf()
    receiver_conf_dct = set_receiver_conf()
    ...
    non_exist_addr = os.environ.get('MAIL_EXTERNAL_RECEIVER_NONEXIST_ACCOUNT')
    try:
        mail_sender.send_mail(sender_conf_dct['addr'], non_exist_addr, 'Test mail', hash)
    except Exception as e:
        if e.smtp_code == 554:
            #print("OK")
        else:
            print("NG")
            print("External Mail Test(External => Internal<non-exist>) Failed:")
            ret = 1
